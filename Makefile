...
int/_constraints.lp: setup.lp seed int
	clingo --verbose=0 --rand-freq=1.0 --seed=`cat seed` --quiet=1,2,2 \
		setup.lp \
		| sed -e "s/) /).\n/g" \
		| grep -v SATISFIABLE \
		| grep -v Optimization \
...
int/_ftree.lp: family.lp int/_constraints.lp seed int
	clingo --verbose=0 --rand-freq=0.05 --seed=`cat seed` --quiet=1,2,2 \
